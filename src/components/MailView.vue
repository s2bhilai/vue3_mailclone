<template>
    <div class="email-display">
        <div>
            <button v-on:click="toggleArchive">{{email.archived ? 'Move to Inbox (e)' : 'Archive (e)'}}</button>
            <button v-on:click="toggleRead">{{email.read ? 'Mark Unread (r)': 'Mark Read (r)'}}</button>
            <button v-on:click="goNewer">Newer (k)</button>
            <button v-on:click="goOlder">Older (j)</button>
        </div>
